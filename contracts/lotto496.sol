...
contract lotto496 is lotto {
	uint8 constant              ballCount           = 49;   // must be under 128
	...
	uint constant               prize4th            = 3;
	uint constant               prize5th            = 2;
	...
	function lottoRoundEnd(uint _seed) internal {
		...
		totalPrize          = prize45(balls, prizeNumbers);                             // 4th	- match 4 numbers, 5th	- match 3 numbers
		tempBalance         = balance-totalPrize;
		totalPrize		    +=prize23(balls, prizeNumbers, bonusNumber, tempBalance);   // 2nd	- match 5 numbers + 1 bonus number, 3rd	- match 5 numbers
    ...
    function prize23(uint128[] _balls, uint128 _prizeNumbers, uint128 _bonusNumber, uint _balance) private returns (uint) {
        uint winner2        = 0;
        uint winner3        = 0;
		...
		for(uint i=0 ; i<ticketsIndex.length ; i++)
            ...
            else if(machine.matchLotto(ticketsIndex[i]&_prizeNumbers,_balls,matchCount-1))
                winner3 +=tickets[ticketsIndex[i]].length;

        uint totalTransfer  = 0;
        uint prize2         = winner2>0?utils.percent(_balance, percent2ndPrize) / winner2 : 0;  // 20% / members
        uint prize3         = winner3>0?utils.percent(_balance, percent3rdPrize) / winner3 : 0;  // 20% / members

		for(i=0 ; i<ticketsIndex.length ; i++)
            if(machine.matchLotto(ticketsIndex[i]&_prizeNumbers,_balls,matchCount-1)&&(ticketsIndex[i]&_bonusNumber>0)&&prize2>0)
			    totalTransfer += prizeX(ticketsIndex[i],prize2);
            else if(machine.matchLotto(ticketsIndex[i]&_prizeNumbers,_balls,matchCount-1)&&prize3>0)
                totalTransfer += prizeX(ticketsIndex[i],prize3);

		return totalTransfer;
	}
	function prize45(uint128[] _balls, uint128 _prizeNumbers) private returns (uint) {
        uint totalTransfer = 0;
        for(uint i=0 ; i<ticketsIndex.length ; i++) {
            uint prize  = machine.matchLotto(ticketsIndex[i]&_prizeNumbers,_balls,matchCount-2)?ticketPrice*prize4th:machine.matchLotto(ticketsIndex[i]&_prizeNumbers,_balls,matchCount-3)?ticketPrice*prize5th:0;
            if(prize>0)
	...
	function buy(uint128[] _tickets) payable isTrue((msg.value == ticketPrice*_tickets.length) && state==STATE.BET) public returns (uint128[]){
        require(machine.validateLottoTicket(_tickets,ballCount,matchCount));    // check 49-6
